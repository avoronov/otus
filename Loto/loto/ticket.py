from .constants import (COLUMNS_COUNT, MAX_NUMBER, MIN_NUMBER, NO_NUMBER,
                        NUMBERS_PER_ROW, NUMBERS_PER_TICKET, ROWS_COUNT)
...
class Cells():
    ...
    def _check(self, numbers):
        ...
        assert len(numbers) == ROWS_COUNT
        total_numbers = 0
        for columns in numbers:
            assert len(columns) == COLUMNS_COUNT
            numbers_per_row = 0
            for column, val in enumerate(columns):
                if val != NO_NUMBER:
                    if val == MAX_NUMBER:
                        assert (val-1) // 10 == column
                    else:
                        assert val // 10 == column
                    total_numbers += 1
                    numbers_per_row += 1
            assert numbers_per_row == NUMBERS_PER_ROW
        assert total_numbers == NUMBERS_PER_TICKET
    ...
    def cell_at(self, row, column):
        ...
        assert check_row_and_column(row, column)
...
class Ticket():
        ...
        for row in numbers:
            ...
            for _ in range(0, NUMBERS_PER_ROW):
                while True:
                    ...
                    if (number not in used_numbers) and (column not in used_columns):
                        row[column] = number
    ...
    def cell_at(self, row, column):
        LOG.debug("Ticket::cell_at(row={}, column={}) started".format(row, column))
    ...
    def find_row_and_column(self, number):
        LOG.debug("Ticket::find_row_and_column(number={}) started".format(number))
    ...
    def is_all_numbers_expunged(self):
        ...
        LOG.debug("Ticket::is_all_numbers_expunged ended with result {}".format(val))
        return val

    def expunge(self, row, column):
        LOG.debug("Ticket::expunge(row={}, column={}) started".format(row, column))
    ...
    def is_correct(self, used_numbers):
        LOG.debug("Ticket::is_correct(used_numbers={}) started".format(used_numbers))
        ...
        if not self.is_all_numbers_expunged():
            ...
            for cell in self._cells:
                LOG.debug("Ticket::is_correct: check cell {}".format(cell))
        ...
        LOG.debug("Ticket::is_correct ended with result {}".format(result))
        return result
